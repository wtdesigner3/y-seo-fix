...
$blogdetail = mysqli_query($conn,"SELECT * FROM projects WHERE `slug` = '$url' AND `status` = '1'");
...
<head>
    ...
    <title><?= $blogdetail['name1'] ?> – Yuccabe Planters</title>
...
<body>
            ...
            <div id="blog-detail-content" class="yb-detail-page">
                <div class="container">
                    ...
                    <div class="yb-detail-breadcrumb">
                        ...
                        <span class="yb-detail-breadcrumb-sep">›</span>
                        <span class="yb-detail-breadcrumb-current"><?= $blogdetail['name1'] ?></span>
                    </div>
                    ...
                    <div class="yb-detail-layout">
                        ...
                        <div class="yb-detail-main">
                            ...
                            <div class="yb-detail-header">
                                ...
                                <h1 class="yb-detail-title"><?= $blogdetail['name1'] ?></h1>
                            </div>

                            <!-- Hero Image -->
                            <div class="yb-detail-hero">
                                <?php
                                    $images = explode(",", $blogdetail['image']);
                                    $singleImage = $images[0];
                                ?>
                                <img src="<?= SITE_URL ?>uploads/<?= $singleImage; ?>" alt="<?= $blogdetail['name1']; ?>" />
                            </div>
                        ...
                        <aside class="yb-detail-sidebar">
                            ...
                            <?php
                        $relatedBlogs = mysqli_query($conn,"SELECT * FROM `projects` WHERE id != ".$blogdetail['id']." AND `status` = '1'");
                        while($relatedBlog = mysqli_fetch_assoc($relatedBlogs)) {
                            ?>
                                    <a href="<?= SITE_URL ?>project/<?= $relatedBlog['slug'] ?>" class="yb-sidebar-card">
                                        <div class="yb-sidebar-thumb">
                                            <?php
                                                $images = explode(",", $relatedBlog['image']);
                                                $singleImage = $images[0];
                                            ?>
                                            <img src="<?= SITE_URL ?>uploads/<?= $singleImage; ?>" alt="<?= $relatedBlog['name1'] ?>" />
                                        </div>
                                        <div class="yb-sidebar-info">
                                            <div class="yb-sidebar-date"><?= $relatedBlog['name1'] ?></div>
                                            <p class="yb-sidebar-title"><?= $relatedBlog['name2'] ?></p>
                                        </div>
                                    </a>
                        <?php
                        }
    ...
    <script src="<?= SITE_URL ?>js/page-load.js"></script>
