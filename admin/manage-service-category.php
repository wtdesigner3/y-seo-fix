...
if (isset($_POST['Dectivate']) && isset($_POST['bb'])) {
	$bb = $_POST['bb'];
	foreach ($bb as $act) {
		mysqli_query($conn, "update categories  set status='0' where id='$act'");
	}
}
...
if (isset($_POST['Activate']) && isset($_POST['bb'])) {
	$bb = $_POST['bb'];
	foreach ($bb as $act) {
		mysqli_query($conn, "update categories  set status='1' where id='$act'");
	}
}
...
if (isset($_POST['Delete']) && isset($_POST['bb'])) {
	$bb = $_POST['bb'];
	foreach ($bb as $act) {
		mysqli_query($conn, "DELETE FROM categories WHERE id=$act");
	}
}


$mqry = "select * from categories  ";
									...
									<table id="data-table-responsive" class="table table-striped table-bordered">
										<thead>
											<tr>
												...
												<th class="text-nowrap">Breadcrumb Heading</th>
												<th width="1%">Sort</th>
										...
										<tbody>
											...
											while ($web = mysqli_fetch_array($fetch)) {
												...
												<tr class="odd gradeX">
													...
													<td>
														<a href="edit-service-category.php?bid=<?php echo $web['id']; ?>"
															class='label label-sm label-primary' title="Edit"><i
																class="fa fa-edit"></i> Edit</a>
													</td>
													<td>
														<a href="delete/service-category.php?bid=<?php echo $web['id']; ?>"
															onClick="if(confirm('Are You Sure Want To Delete This Record')){ return true;} else { return false; }"
