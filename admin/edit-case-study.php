...
if(isset($_POST['update']))
{
  ...
  $query=mysqli_query($conn,"UPDATE `tbl_case_study` SET `b_title`='$title',`b_description`='$desc',`b_status`='$status',`b_sort`='$sort' WHERE `b_id`='$b'");
  if($query==true)
  {
			...
			<form role="form"  method="POST"  enctype="multipart/form-data">
              <div class="box-body">
                ...
                <div class="form-group">
                  <label for="bannerlink"> Faq Title</label>
                  <input type="text" name="title" class="form-control" value="<?= $brec['b_title']; ?>" >
                </div>                
               
                <div class="form-group">
                  <label for="bannerlink">Faq Description</label>
                  <textarea  name="desc"  placeholder="Enter  Description" rows="5" class="form-control" ><?=$brec['b_description']?></textarea>
                </div>

                <div class="form-group">
                  <label for="bannerlink"> Faq Sort</label>
                  <input type="text" name="sort" class="form-control" value="<?= $brec['b_sort']; ?>" >
                </div>                   
